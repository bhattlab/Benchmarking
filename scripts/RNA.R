...
dummy
b <- get_legend(dummy)
stacked <- plot_grid(r, b, nrow=2, ncol=1, rel_heights = c(1, 0.08), rel_widths = c(1, 1))
stacked
##### SHANNON DIVERSITY #####
#Change column name
...
bact <- ggplot(raw , aes(x=reorder(Sample_Type, PlotOrder), Relative.Abundance..Bacteroidetes)) + 
  ...
  stat_pvalue_manual(sig %>% filter(feature == "Relative Abundance: Bacteroidetes") %>% filter(p.adj <= 0.05), y.position=c(0.9, 0.95, 0.85),
                     tip.length=0, label = "p.signif") +
  theme_bw() + 
  ylim(0,1) + 
...
act <- ggplot(raw , aes(reorder(Sample_Type, PlotOrder), Relative.Abundance..Actinobacteria)) + 
  ...
  stat_pvalue_manual(sig %>% filter(feature == "Relative Abundance: Actinobacteria") %>% filter(p.adj <= 0.05), y.position=c(0.27, 0.22, 0.17), 
                     tip.length=0, label = "p.signif") +
  theme_bw() + 
  ylim(0,1) + 
  ylab("Relative Abundance") + 
...
#Plot Figure 4
a <- plot_grid(stacked,shannon, nrow=1, ncol=2, scale=0.9, rel_widths=c(1,0.3), labels=c("a","b"))
a
b <- plot_grid(richness,bc_full,pdiff, nrow=1, ncol=3, scale=0.9, rel_widths=c(0.8,0.8,1), labels=c("c","d","e"))
b

four <- plot_grid(a,b,nrow=2,ncol=1)
four

ggsave(here("QSU_Data/Figure4.pdf"), dpi=300, w=15, h=8)
ggsave(here("QSU_Data/Figure4.jpeg"), dpi=300, w=14, h=8)

#### EXTRA ####
#Stacked Bar Plot for Donor 1 only
